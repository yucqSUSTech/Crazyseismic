function [ rayp ] = y_get_rayp_PKIKP( evdp, em, dist )
...
zt= wise_turn_v3 (pmin, [em.z_iob em.re],  z_fine, sp_fine);
[rtmp1, dtmp1]=  tau (evdp, zt ,pmin, z_fine, sp_fine);
[rtmp2, dtmp2]= tau ( 0.0, zt,pmin, z_fine, sp_fine);
dtaup = dtmp1 + dtmp2;
range1 = dtaup*180/pi;
zt= wise_turn_v3 (pmax, [em.z_iob em.re],  z_fine, sp_fine);
[rtmp1, dtmp1]=  tau (evdp, zt ,pmax, z_fine, sp_fine);
[rtmp2, dtmp2]= tau ( 0.0, zt,pmax, z_fine, sp_fine);
dtaup = dtmp1 + dtmp2;
range2 = dtaup*180/pi;

if isempty(dist)
    rayp = linspace(pmin,pmax,11);
    return;
...
for i = 1:length(dist)
    ...
    while ddist > 0.001 && num < 30 % ~0.1 km offset
        num = num + 1;
        pj = (pmax + pmin)/2;
        zt= wise_turn_v3 (pj, [em.z_iob em.re],  z_fine, sp_fine);
