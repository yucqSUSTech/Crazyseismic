function  [rayp, taup1, dtaup1]= y_get_sPn (evdp, np, em)
...
if evdp > em.z_moho
    fprintf('Event below Moho, No sPn phase');
    rayp = zeros(1,np);
    taup1 = nan(1,np);
...
% zt = zt - 0.5*10^-6;
zt = em.z_moho;
[rtmp1, dtmp1]= tau (0.0, evdp, pmin, em.z, ss_fine);
[rtmp2, dtmp2]= tau (0.0, zt, pmin, em.z, sp_fine);
