function [tab, dtab, dd, zz, etab, edtab ]= tau_raypath_v2 (za, zb, p0, zi, si) 
% % 
% % 
...
% also output tab and dtab in each step: etab and edtab
...
dd = zeros(jb-ja+2,1);
zz = zeros(jb-ja+2,1);
etab = zeros(jb-ja+2,1);
edtab = zeros(jb-ja+2,1);
dd(num) = 0;
zz(num) = znew(ja);
etab(num) = 0;
edtab(num) = 0;
...
%     [tjx, djx,ttjx,dtau2]= tauj_n ( sj2, sj1, re-zj2, re-zj1, p0 ); 
  ...
  num = num + 1; % add by yucq 10/11/11
  dd(num) = djx + dd(num-1);
  zz(num) = zj2;
  etab(num) = tjx;
  edtab(num) = djx;
