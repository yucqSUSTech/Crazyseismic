function [ rayp ] = y_get_rayp_SxS( evdp, em, dist, np, xdep )
...
[rtmp3, dtmp3]= tau ( 0.0, zt,pmin, z_fine, ss_fine);
...
[rtmp3, dtmp3]= tau ( 0.0, zt,pmax, z_fine, ss_fine);
dtaup = dtmp1 + 2*dtmp2 + dtmp3;
range2 = dtaup*180/pi;
...
for i = 1:length(dist)
    ...
    while ddist > 0.001 && num < 30 % ~0.1 km offset
        ...
        [rtmp3, dtmp3]=  tau ( 0.0, zt,pj, z_fine, ss_fine);
        dtaup = dtmp1 + 2*dtmp2 +dtmp3;
