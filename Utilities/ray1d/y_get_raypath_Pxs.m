function [ zdt  ] = y_get_raypath_directP( evdp, rayp, em, xdep )
...
for i = 1:length(rayp)
    ...
    [rtmp2,dtmp2,dd2,zz2,etab2,edtab2] = tau_raypath_v2(xdep,zt,pj,z_fine,sp_fine);
    [rtmp3,dtmp3,dd3,zz3,etab3,edtab3] = tau_raypath_v2(0,xdep,pj,z_fine,ss_fine);
    ...
    tt3 = tt2(end)-flipud(tt3);
    zz3 = flipud(zz3);
    dd3 = dd3(end)-flipud(dd3);

    zdt(i).part(1).tt = real([tt1;tt1(end)+tt2(2:end)]);
    zdt(i).part(2).tt = real(tt1(end)+tt2(end)+tt3);
    zdt(i).part(1).dd = 180/pi*real([dd1;dd1(end)+dd2(2:end)]);
    zdt(i).part(2).dd = 180/pi*real(dd1(end)+dd2(end)+dd3);
    zdt(i).part(1).zz = real([zz1;zz2(2:end)]);
    zdt(i).part(2).zz = real(zz3);
