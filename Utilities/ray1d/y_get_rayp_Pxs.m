function [ rayp ] = y_get_rayp_Pxs( evdp, em, dist, np, xdep )
...
ztmp = max( [em.z_660 , evdp, xdep]) + 1 ; 
[pmax,iflag] =  interp1db ( ztmp, z_fine, sp_fine );

% calculat the maximum and minimum distance range
zt= wise_turn_v3 (pmin, [em.z_660 em.z_cmb],  z_fine, sp_fine);
[rtmp1, dtmp1]=  tau (evdp, zt ,pmin, z_fine, sp_fine);
[rtmp2, dtmp2]= tau ( xdep, zt,pmin, z_fine, sp_fine);
[rtmp3, dtmp3]= tau ( 0.0, xdep,pmin, z_fine, ss_fine);
dtaup = dtmp1 + dtmp2 + dtmp3;
range1 = dtaup*180/pi;
zt= wise_turn_v3 (pmax, [em.z_660 em.z_cmb],  z_fine, sp_fine);
[rtmp1, dtmp1]=  tau (evdp, zt ,pmax, z_fine, sp_fine);
[rtmp2, dtmp2]= tau ( xdep, zt,pmax, z_fine, sp_fine);
[rtmp3, dtmp3]= tau ( 0.0, xdep,pmax, z_fine, ss_fine);
dtaup = dtmp1 + dtmp2 + dtmp3;
range2 = dtaup*180/pi;
...
for i = 1:length(dist)
    ...
    while ddist > 0.001 && num < 30 % ~0.1 km offset
        ...
        [rtmp2, dtmp2]= tau ( xdep, zt,pj, z_fine, sp_fine);
        [rtmp3, dtmp3]= tau ( 0.0, xdep,pj, z_fine, ss_fine);
