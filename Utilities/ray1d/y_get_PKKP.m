function  [rayp, taup1, dtaup1]= y_get_PKKP (evdp, np, em)
...
% for K waves
ztmp = em.z_cmb + 10^-6;
[pKmax,iflag] =  interp1db ( ztmp, em.z, sp_fine );
ztmp = em.z_iob - 10^-6;
[pKmin,iflag] =  interp1db ( ztmp, em.z, sp_fine );

% find the maximum and minimum rayp for the phase
pmax = min(pKmax,pPmax);
pmin = max(pKmin,pPmin);
...
for j =1: np
    ...
    zt = zt - 0.5*10^-6;
    [rtmp1, dtmp1]= tau (evdp, em.z_cmb ,pj, em.z, sp_fine);
    [rtmp2, dtmp2]= tau (em.z_cmb,zt,pj, em.z, sp_fine);
    [rtmp3, dtmp3]= tau (0,evdp,pj, em.z, sp_fine);
