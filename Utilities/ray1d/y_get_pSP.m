function  [rayp, taup1, dtaup1]= y_get_pSP (evdp, np, em)
...
for j =1: np
    ...
    zt_p = zt_p - 0.5*10^-6;
    [rtmp0, dtmp0]= tau ( 0.0, evdp ,pj, em.z, sp_fine);
    ...
    taup1(j) = rtmp0 + 2*rtmp1 + 2*rtmp2 + 2*rtmp3;
    dtaup1(j) = dtmp0 + 2*dtmp1 + 2*dtmp2 + 2*dtmp3;
end
