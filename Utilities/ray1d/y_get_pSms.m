function  [rayp, taup1, dtaup1]= y_get_pSms (evdp, np, em)
...
if evdp > em.z_moho
    fprintf('Event below Moho, No pSms phase');
    rayp = zeros(1,np);
    taup1 = nan(1,np);
...
[pmax2,iflag] = interp1db ( evdp, em.z,sp_fine);
...
for j =1: np
    ...
    [rtmp1, dtmp1]= tau (0.0, evdp ,pj, em.z, sp_fine);
